...
status_t C2OMXNode::getParameter(OMX_INDEXTYPE index, void *params, size_t size) {
    ...
    switch ((uint32_t)index) {
        ...
        case OMX_IndexParamPortDefinition: {
            ...
            pDef->nBufferCountActual = bufferParams.mBufferCountActual;
            pDef->eDomain = OMX_PortDomainVideo;
            pDef->format.video.nFrameWidth = bufferParams.mFrameWidth;
            pDef->format.video.nFrameHeight = bufferParams.mFrameHeight;
...
status_t C2OMXNode::emptyBuffer(
    ...
    if (omxBuf.mBufferType == OMXBuffer::kBufferTypeANWBuffer
        ...
        return mImpl->emptyBuffer(buffer, omxBuf.mGraphicBuffer, toNodeFlags(flags),
                                  toNodeTimestamp(timestamp), fenceFd);
    }
    sp<GraphicBuffer> gBuf;
    return mImpl->emptyBuffer(buffer, gBuf, toNodeFlags(flags),
                              toNodeTimestamp(timestamp), fenceFd);
}
