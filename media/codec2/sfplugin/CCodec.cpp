...
public:
    ...
    status_t connect(const std::shared_ptr<Codec2Client::Component> &comp) override {
        ...
        params.resize(sizeof(usage));
        ::memcpy(&params[0], &usage, sizeof(usage));
        (void)mNode->setParameter(IndexConsumerUsageBits, params);
    ...
    status_t start() override {
        ...
        C2NodeInputBufferParams param;
        std::vector<uint8_t> inParams;
        std::vector<uint8_t> outParams;
        inParams.resize(sizeof(param));
        ::memcpy(&inParams[0], &param, sizeof(param));
        status_t err = fromAidlStatus(mNode->getParameter(
                IndexInputBufferParams, inParams, &outParams));
        if (err == OK) {
            C2NodeInputBufferParams *pOutParam =
                    (C2NodeInputBufferParams *)&outParams[0];
            numSlots = pOutParam->mBufferCountActual;
        }
    ...
    status_t configure(Config &config) {
        ...
        if (config.mMinAdjustedFps > 0 || config.mFixedAdjustedFps > 0) {
            if (mNode != nullptr) {
                ...
                std::vector<uint8_t> params;
                params.resize(sizeof(gapUs));
                ::memcpy(&params[0], &gapUs, sizeof(gapUs));
                (void)mNode->setParameter(IndexAdjustTimestamp, params);
            }
        }
