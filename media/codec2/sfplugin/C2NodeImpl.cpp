...
void C2NodeImpl::getInputBufferParams(C2NodeInputBufferParams *params) {
    params->mBufferCountActual = 16;
    ...
    if (mWidth * mHeight > 4096 * 2340) {
        ...
        if (c2err == C2_OK || c2err == C2_BAD_INDEX) {
            params->mBufferCountActual = 4;
            params->mBufferCountActual += (inputDelay ? inputDelay.value : 0u);
            params->mBufferCountActual += (pipelineDelay ? pipelineDelay.value : 0u);
        }
    }

    params->mFrameWidth = mWidth;
    params->mFrameHeight = mHeight;
}
...
status_t C2NodeImpl::emptyBuffer(
