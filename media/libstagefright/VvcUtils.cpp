...
#define MAX_NUM_SUB_PICS                      600    // Spec, Level 6.0

namespace android {

// Refer to ISO/IEC 14496-15:2021(E)
// Chapter 11.2.4.2.1
// It is recommended that the arrays be in the order DCI, OPI, VPS, SPS, PPS, prefix APS, prefix SEI.
...
status_t VvcParameterSets::makeVvcc(uint8_t *vvcc, size_t *vvccSize, size_t nalSizeLength) {
    ...
    size_t size = 15;
    if (mPTRRecord != NULL) {
        // mPTRRecord doesn't contains 	bit(2) reserved = 0 and unsigned int(6) num_bytes_constraint_info
    ...
    if (!findParam8(kNumSubLayers, &num_sublayers) || !findParam8(kChromaFormatIdc, &chroma_format_idc)
        || !findParam8(kBitDepthChromaMinus8,  &bit_depth_minus8) || !findParam32(kMaxPictureWidth, &max_picture_width)
        || !findParam32(kMaxPictureHeight, &max_picture_height) || !findParam32(kNumBytesConstraintInfo, &num_bytes_constraint_info)) {
    ...
    *vvccSize = size;
    ...
    header[6] = 0x04 | (num_sublayers << 4) | chroma_format_idc; // set constant_frame_rate to 1 currently
...
status_t VvcParameterSets::parseSps(const uint8_t* data, size_t size) {
    ...
    ALOGV("sps_max_sublayers_minus1:%u, sps_chroma_format_idc: %u", sps_max_sublayers_minus1, sps_chroma_format_idc);
    ...
    if (sps_ptl_dpb_hrd_params_present_flag) {
        ...
        profile_tier_level(&reader, 1, sps_max_sublayers_minus1);
    ...
    ALOGV("sps_pic_width_max_in_luma_samples:%u, sps_pic_height_max_in_luma_samples:%u, tmpWidthVal:%u, tmpHeightVal:%u, CtbSizeY: %u,",
            sps_pic_width_max_in_luma_samples, sps_pic_height_max_in_luma_samples, tmpWidthVal, tmpHeightVal, CtbSizeY);
    ...
    if (reader.getBitsWithFallback(1, 0)) { //sps_subpic_info_present_flag
        ...
        // uint32_t sps_subpic_ctu_top_left_x[MAX_NUM_SUB_PICS];
        // uint32_t sps_subpic_ctu_top_left_y[MAX_NUM_SUB_PICS];
        // uint32_t sps_subpic_width_minus1[MAX_NUM_SUB_PICS];
        // uint32_t sps_subpic_height_minus1[MAX_NUM_SUB_PICS];
    ...
    if (sps_ptl_dpb_hrd_params_present_flag) {
        ...
        dpb_parameters(&reader, sps_max_sublayers_minus1, sps_sublayer_dpb_params_flag);
    }
    ...
    for (uint32_t i = 0; i < (sps_rpl1_same_as_rpl0_flag ? 1 : 2); i++) {
        ...
        for (uint32_t j = 0; j < sps_num_ref_pic_lists; j++) {
            ref_pic_list_struct(&reader, i, j, sps_num_ref_pic_lists,
                                    sps_long_term_ref_pics_flag,
                                    sps_inter_layer_prediction_enabled_flag,
        ...
        if (reader.getBitsWithFallback(1, 0)) { // sps_timing_hrd_params_present_flag
            ...
            for (uint32_t i = firstSubLayer; i <= sps_max_sublayers_minus1; i++) {
                ...
                if (general_nal_hrd_params_present_flag) {
                    sublayer_hrd_parameters(&reader, i, general_du_hrd_params_present_flag, hrd_cpb_cnt_minus1);
                }
...
void VvcParameterSets::profile_tier_level(NALBitReader* br, uint32_t profileTierPresentFlag, int MaxNumSubLayersMinus1) {
...
void VvcParameterSets::general_constraints_info(NALBitReader* br) {
    if (br == NULL) {
        return;
    }
...
void VvcParameterSets::dpb_parameters(ABitReader* br, uint32_t MaxSubLayersMinus1, uint32_t subLayerInfoFlag)  {
...
void VvcParameterSets::ref_pic_list_struct(ABitReader* br,
...
void VvcParameterSets::sublayer_hrd_parameters(ABitReader* br,
