...
static void parseVvcProfileLevelFromVvcc(const uint8_t *ptr, size_t size, sp<AMessage> &format) {
    ...
    bool ptl_present_flag = br.getBits(1); // unsigned int(1) ptl_present_flag;
    ...
    br.skipBits(6);
    const uint8_t profile = br.getBits(7); // unsigned int(7) general_profile_idc;
    const uint8_t tier = br.getBits(1); // unsigned int(1) general_tier_flag;
    const uint8_t level = br.getBits(8); // unsigned int(8) general_level_idc;
...
status_t convertMessageToMetaData(const sp<AMessage> &msg, sp<MetaData> &meta) {
    ...
    } else if (mime == "video/vvc" && msg->findBuffer("csd-vvc", &csd0)) {
