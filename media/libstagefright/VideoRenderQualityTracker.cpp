...
void VideoRenderQualityTracker::onFrameRendered(int64_t contentTimeUs, int64_t actualRenderTimeNs,
    ...
    while (!mNextExpectedRenderedFrameQueue.empty()) {
        ...
        if (!mPendingSkippedFrameContentTimeUsList.empty()) {
            while (mPendingSkippedFrameContentTimeUsList.front()
                        <= nextExpectedFrame.contentTimeUs) {
    ...
    if (!mPendingSkippedFrameContentTimeUsList.empty()) {
        while (mPendingSkippedFrameContentTimeUsList.front() < nextExpectedFrame.contentTimeUs) {
            processMetricsForSkippedFrame(mPendingSkippedFrameContentTimeUsList.front());
            mPendingSkippedFrameContentTimeUsList.pop_front();
        }
    }
