  ...
  while (!saw_output_eos_ && !saw_error_) {
    ...
    if (buffer_info_size <= 0) {
      if (buffer_info_with_index.index < 0) {
        ALOGI("That was an empty buffer to mark error, nothing to release");
      } else {
    ...
    if (buffer_info_with_index.buffer_info.flags & AMEDIACODEC_BUFFER_FLAG_END_OF_STREAM) {
      saw_output_eos_ = true;
    }
    ++output_frame_count_;
  }
...
void MediaCodecDecoderNdk::onAsyncError(media_status_t error,
                                        ...
                                        const char* detail) {
  saw_error_ = true;
  BufferInfoWithIndex emptyBuffer;
  available_output_buffers_.push(emptyBuffer);
