...
void testAsyncCodec(AMediaCodec* decoder, const char* mime, bool setAsync) {
  using namespace std::chrono_literals;
  // Setup decoder callbacks.
  AMediaCodecOnAsyncNotifyCallback callbacks = {nullptr, nullptr, nullptr, nullptr};

  if (setAsync) {
    callbacks.onAsyncInputAvailable = [](AMediaCodec* codec,
                                         void* userdata,
                                         int32_t index) {
      ALOGI("GIRISH: codec: %p userdata: %p index: %d", codec, userdata, index);
      bool* received = static_cast<bool*>(userdata);
      *received = true;
    };

    callbacks.onAsyncOutputAvailable = [](AMediaCodec* codec,
                                          void* userdata,
                                          int32_t index,
                                          AMediaCodecBufferInfo* info) {
      ALOGI("codec: %p userdata: %p index: %d info: %p", codec, userdata, index, info);
    };

    callbacks.onAsyncFormatChanged = [](AMediaCodec* codec,
                                        void* userdata,
                                        AMediaFormat* format) {
      ALOGI("codec: %p userdata: %p format: %p", codec, userdata, format);
    };

    callbacks.onAsyncError = [](AMediaCodec* codec,
                                void* userdata,
                                media_status_t error,
                                int32_t code,
                                const char* details) {
      ALOGI("codec: %p userdata: %p Error(%d:%d:%s)", codec, userdata, error, code, details);
    };
  }

  bool received = false;
  ALOGI("GIRISH: AMediaCodec_setAsyncNotifyCallback %s", setAsync? "true" : "false");
  media_status_t err = AMediaCodec_setAsyncNotifyCallback(decoder, callbacks, &received);
  ASSERT_EQ(err, AMEDIA_OK) << "AMediaCodec_setAsyncNotifyCallback is expected to Succeed";
  ...
  err = AMediaCodec_start(decoder);
  ASSERT_EQ(err, AMEDIA_OK) << "Start is expected to Succeed";

  // Run this loop for a second.
  // Since each iteration blocks for 100ms, the loop is
  // run for 10 times.
  int attempts = 10;
  while (--attempts > 0) {
    ssize_t index = AMediaCodec_dequeueInputBuffer(decoder, 0);
    ALOGI("GIRISH: AMediaCodec_dequeueInputBuffer returned: %zd", index);
    if (setAsync) {
      // When asynchronous callback is enabled, calling
      // AMediaCodec_dequeueInputBuffer should return
      // AMEDIA_ERROR_INVALID_OPERATION.
      ASSERT_EQ(index, AMEDIA_ERROR_INVALID_OPERATION)
        << "AMediaCodec_dequeueInputBuffer is expected to fail";
      if (received) {
        // If we have already received input buffer available callback end the loop.
        break;
      }
    } else {
      // When asynchronous callback is disabled, calling
      // AMediaCodec_dequeueInputBuffer should return either a valid
      // (input) buffer index or AMEDIACODEC_INFO_TRY_AGAIN_LATER.
      bool success = (index == AMEDIACODEC_INFO_TRY_AGAIN_LATER || index >= 0);
      ASSERT_TRUE(success) << "AMediaCodec_dequeueInputBuffer is expected to Succeed";
      // Since we have unset the asynchronous callback, we shouldn't be
      // receiving any async callbacks from codec.
      ASSERT_FALSE(received) << "Not suppose to receive async callbacks";
    }
    // Wait for 100ms and try again.
    std::this_thread::sleep_for(100ms);
  }

  err = AMediaCodec_stop(decoder);
  ASSERT_EQ(err, AMEDIA_OK) << "Stop is expected to Succeed";

  AMediaFormat_delete(decoderFormat);
}

TEST(NdkMediaCodec_tests, codec_async_sync) {
  // Test scenario:
  //
  // 1. Create and configure a decoder in async mode.
  // 2. After starting the decoder, the test ensures that
  // - async callbacks for input buffer availability is issued
  // - calling restricted APIs while on async mode gives error code.
  // 3. Stop and reconfigure the same decoder in sync mode by
  //    unsetting the async callbacks.
  // 4. After starting the decoder, the test ensures that
  // - No more async callbacks isues on the decoder
  // - Input buffer should be available through AMediaCodec_dequeueInputBuffer
  ...
  // Configure the decoder in async mode and verify the functionalities.
  testAsyncCodec(decoder, mime, true);
  // Unset the async callbacks, re-ronfigure the decoder and verify the functionalities.
  testAsyncCodec(decoder, mime, false);
