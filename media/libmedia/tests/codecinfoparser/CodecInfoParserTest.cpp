...
static void verifyInfoParserResults(sp<MediaCodecInfo> &info, bool isEncoder) {
    ...
    for (AString mediaType : mediaTypes) {
        ...
        std::shared_ptr<MediaCodecInfoParser::AudioCapabilities> audioCap = codecCap.getAudioCapabilities();
        std::shared_ptr<MediaCodecInfoParser::VideoCapabilities> videoCap = codecCap.getVideoCapabilities();
        std::shared_ptr<MediaCodecInfoParser::EncoderCapabilities> encoderCap = codecCap.getEncoderCapabilities();

        if (audioCap != nullptr) {
            Range<int> bitrateRange = audioCap->getBitrateRange();
            std::vector<int> supportedSampleRates = audioCap->getSupportedSampleRates();
            std::vector<Range<int>> supportedSampleRateRanges = audioCap->getSupportedSampleRateRanges();
            int maxInputChannelCount = audioCap->getMaxInputChannelCount();
            int minInputChannelCount = audioCap->getMinInputChannelCount();
            std::vector<Range<int>> inputChannelCountRanges = audioCap->getInputChannelCountRanges();
        }
    }
}
