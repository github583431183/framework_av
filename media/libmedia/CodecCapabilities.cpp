...
bool CodecCapabilities::isFormatSupported(const sp<AMessage> &format) const {
    ...
    if (strcasecmp(mMediaType.c_str(), mediaType.c_str()) != 0) {
        return false;
    }
    ...
    if (format->findInt32(KEY_PROFILE, &profile)) {
        ...
        for (ProfileLevel pl : mProfileLevels) {
            if (pl.mProfile == profile && pl.mLevel > maxLevel) {
                ...
                if ((strcasecmp(mMediaType.c_str(), MIMETYPE_VIDEO_H263) != 0)
...
bool CodecCapabilities::supportsProfileLevel(int profile, int level) const {
    for (ProfileLevel pl: mProfileLevels) {
        ...
        if (strcasecmp(mMediaType.c_str(), MIMETYPE_AUDIO_DTS) == 0
                || strcasecmp(mMediaType.c_str(), MIMETYPE_AUDIO_DTS_HD) == 0
                || strcasecmp(mMediaType.c_str(), MIMETYPE_AUDIO_DTS_UHD) == 0) {
            return true;
        }
        ...
        if (strcasecmp(mMediaType.c_str(), MIMETYPE_VIDEO_MPEG4) == 0) {
        ...
        if (strcasecmp(mMediaType.c_str(), MIMETYPE_VIDEO_HEVC) == 0) {
...
std::string CodecCapabilities::getMediaType() {
    return mMediaType;
}
...
std::shared_ptr<CodecCapabilities> CodecCapabilities::CreateFromProfileLevel(
        std::string mediaType, int profile, int level, int32_t maxConcurrentInstances) {
    ProfileLevel pl;
    pl.mProfile = profile;
    pl.mLevel = level;
...
int CodecCapabilities::getErrors() const {
    if (mAudioCaps) {
