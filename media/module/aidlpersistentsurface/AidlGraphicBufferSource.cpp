...
::ndk::ScopedAStatus AidlGraphicBufferSource::onExecuting() {
    status_t err = start();
    return (OK == err) ? ::ndk::ScopedAStatus::ok() :
            ::ndk::ScopedAStatus::fromServiceSpecificError(err);
}

::ndk::ScopedAStatus AidlGraphicBufferSource::onIdle() {
    status_t err = stop();
    return (OK == err) ? ::ndk::ScopedAStatus::ok() :
            ::ndk::ScopedAStatus::fromServiceSpecificError(err);
}

::ndk::ScopedAStatus AidlGraphicBufferSource::onLoaded(){
