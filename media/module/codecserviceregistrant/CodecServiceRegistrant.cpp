...
static android::sp<c2_hidl_V1_0::IComponentStore> getDeclaredHidlSwcodec(const std::shared_ptr<C2ComponentStore>& store) {
...
extern "C" void RegisterCodecServices() {
    ...
    if (__builtin_available(android __ANDROID_API_S__, *)) {
        if (AServiceManager_isDeclared(aidlServiceName.c_str())) {
            std::shared_ptr<c2_aidl::IComponentStore> aidlStore = ::ndk::SharedRefBase::make<c2_aidl::utils::ComponentStore>(
    ...
    if (hidlStore) {
        if (hidlStore->registerAsService("software") == android::OK) {
            ...
            LOG(ERROR) << "Cannot register software Codec2" << hidlStore->descriptor << " service.";
        }
    } else {
