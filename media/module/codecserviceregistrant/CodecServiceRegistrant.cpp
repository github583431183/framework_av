...
extern "C" void RegisterCodecServices() {
    ...
    // look for software component in the manifest; instantiate the most recent
    // HIDL version we find.
    // NB: we don't continue to the next one if instantiation fails on the first one.
    bool foundHidl = false;
    if (!foundHidl) {
        using ::android::hidl::manager::V1_2::IServiceManager;
        IServiceManager::Transport transport_v1_2 =
                android::hardware::defaultServiceManager1_2()->getTransport(
                        V1_2::utils::ComponentStore::descriptor, "software");
        if (transport_v1_2 == IServiceManager::Transport::HWBINDER) {
            foundHidl = true;
            if (!hidlStore) {
            ...
            if (hidlStore->registerAsService("software") == android::OK) {
                ...
                LOG(ERROR) << "Cannot register software Codec2 HIDL v" << hidlVer << " service.";
            }
        }
    }
    if (!foundHidl) {
        using ::android::hidl::manager::V1_1::IServiceManager;
        IServiceManager::Transport transport_v1_1 =
                android::hardware::defaultServiceManager1_1()->getTransport(
                        V1_1::utils::ComponentStore::descriptor, "software");
        if (transport_v1_1 == IServiceManager::Transport::HWBINDER) {
            foundHidl = true;
            if (!hidlStore) {
                hidlStore = ::android::sp<V1_1::utils::ComponentStore>::make(
                        std::make_shared<H2C2ComponentStore>(nullptr));
                hidlVer = "1.1";
            }
            if (hidlStore->registerAsService("software") == android::OK) {
