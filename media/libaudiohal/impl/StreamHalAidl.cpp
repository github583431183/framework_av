        ...
        }
        {
            ...
            if (!mIsInput && reply->status == STATUS_OK) {
                if (command.getTag() == StreamDescriptor::Command::standby &&
                    ...
                    mStatePositions.framesAtFlush = reply->observable.frames;
                }
            }
...
status_t StreamOutHalAidl::getRenderPosition(uint64_t *dspFrames) {
    ...
    const int64_t mostRecentResetPoint = !mContext.isAsynchronous() ? statePositions.framesAtStandby
            // For compressed and offload streams the frame count must also reset after flush.
            : std::max(statePositions.framesAtStandby, statePositions.framesAtFlush);
...
status_t StreamOutHalAidl::getPresentationPosition(uint64_t *frames, struct timespec *timestamp) {
    ...
    if (!mContext.isAsynchronous()) {
        *frames = aidlFrames;
        ...
        // For compressed and offload streams the frame count must reset after flush,
        // also, it is recommended that HAL resets the frame count when the stream enters STANDBY.
