...
status_t Hal2AidlMapper::findOrCreateMixPortConfig(
    ...
    if (auto portConfigIt = findPortConfig(config, flags, ioHandle);
        ...
        std::vector<AudioInputFlags> kOptionalInputFlags {
        ...
        std::vector<AudioOutputFlags> kOptionalOutputFlags { };
        ...
        if (mRemoteSubmixOut.has_value()) {
            kOptionalOutputFlags.push_back(AudioOutputFlags::DIRECT);
            kOptionalOutputFlags.push_back(AudioOutputFlags::IEC958_NONAUDIO);
        }
        ...
        while (portsIt == mPorts.end() && matchFlags.getTag() == AudioIoFlags::Tag::output
                && optionalOutputFlagsIt != kOptionalOutputFlags.end()) {
            if (!isBitPositionFlagSet(
