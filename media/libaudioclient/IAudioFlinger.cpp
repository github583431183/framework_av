...
status_t AudioFlingerClientAdapter::reset() {
    RETURN_STATUS_IF_ERROR(statusTFromBinderStatus(mDelegate->reset()));
    return OK;
}
...
Status AudioFlingerServerAdapter::reset() {
    RETURN_BINDER_IF_ERROR(mDelegate->reset());
    return Status::ok();
}
