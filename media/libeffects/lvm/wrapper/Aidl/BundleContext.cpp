...
RetCode BundleContext::setForcedDevice(
    ...
    if (isDeviceSupportedVirtualizer({device})) {
        ...
        if (device != AudioDeviceDescription{AudioDeviceType::NONE}) {
        ...
        mVirtualizerForcedDevice = AudioDeviceDescription{AudioDeviceType::NONE};
    }
    ...
    if (enableVirtualizer) {
        if (mVirtualizerTempDisabled) {
            ...
            LOG(VERBOSE) << __func__ << " leaving virtualizer enabled";
        }
    } else {
        if (!mVirtualizerTempDisabled) {
            ...
            LOG(VERBOSE) << __func__ << " leaving virtualizer disabled";
        }
    }
