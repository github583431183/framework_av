...
status_t Engine::loadFromHalConfigWithFallback(
    ...
    return loadWithFallback(aidlConfig, std::move(capResult));
}

status_t Engine::loadFromXmlConfigWithFallback(const std::string& xmlFilePath)
{
    capEngineConfig::ParsingResult result = capEngineConfig::parse();
    if (result.parsedConfig == nullptr) {
        ALOGE("%s: There was an error parsing AIDL data", __func__);
        return NO_INIT;
    }
    return loadWithFallback(xmlFilePath, std::move(result));
}

template<typename T>
status_t Engine::loadWithFallback(const T& configSource,
        const android::capEngineConfig::ParsingResult&& capSettings __unused) {
    ...
    mPolicyParameterMgr->setConfiguration(std::move(capSettings));
