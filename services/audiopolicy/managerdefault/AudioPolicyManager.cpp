                                                       ...
                                                       audio_devices_t device)
{
    ...
    if (!curCurvAttrs.empty() && curCurvAttrs.front() != defaultAttr) {
        ...
        DeviceVector devices;
        getDevicesForAttributes(attr, uid, devices, /* preferredDevices*/ nullptr,
                                /* forVolume */ true);
...
audio_direct_mode_t AudioPolicyManager::getDirectPlaybackSupport(const audio_attributes_t *attr,
    ...
    DeviceVector engineOutputDevices;
    getDevicesForAttributes(*attr, uid, engineOutputDevices);
    for (const auto& hwModule : mHwModules) {
...
status_t AudioPolicyManager::connectAudioSource(const sp<SourceClientDescriptor>& sourceDesc)
{
    ...
    getDevicesForAttributes(attributes, sourceDesc->uid(), sinkDevices);
...
void AudioPolicyManager::checkVirtualizerClientRoutes() {
    ...
    for (size_t i = 0; i < mOutputs.size(); i++) {
        ...
        for (const sp<TrackClientDescriptor>& client : desc->getClientIterable()) {
            ...
            DeviceVector devices;
            getDevicesForAttributes(attr, client->uid(), devices);
...
status_t AudioPolicyManager::getDevicesForAttributes(
    ...
    if (status_t status = getDevicesForAttributes(attr, uid, curDevices,
                /* preferredDevices= */ nullptr, forVolume); status != OK) {
        return status;
    }
...
bool AudioPolicyManager::areAllActiveTracksRerouted(const sp<SwAudioOutputDescriptor>& output)
{
    ...
    for (const auto& client : activeClients) {
        ...
        sp<DeviceDescriptor> preferredDevice =
                mAvailableOutputDevices.getDeviceFromId(client->preferredDeviceId());
        DeviceVector devices;
        getDevicesForAttributes(client->attributes(), client->uid(), devices, preferredDevice);
        if (devices == routedDevices) {
            return false;
        }
    }
    return true;
...
status_t AudioPolicyManager::getDevicesForAttributes(
        ...
        const sp<DeviceDescriptor> &preferredDevice, bool forVolume) {
    ...
    if (policyMix != nullptr && policyMix->getOutput() != nullptr &&
        ...
        devices = mEngine->getOutputDevicesForAttributes(
                attr, uid, preferredDevice, false /* fromCache */);
    }
