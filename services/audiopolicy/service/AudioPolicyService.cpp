...
void AudioPolicyService::AudioCommandThread::insertCommand_l(sp<AudioCommand>& command, int delayMs)
{
    ...
    for (i = (ssize_t)mAudioCommands.size()-1; i >= 0; i--) {
        ...
        case SET_PORTS_VOLUME: {
            ...
            std::vector<audio_port_handle_t> fromCommand2OnlyPorts{};
            std::copy_if(data2->mPorts.begin(), data2->mPorts.end(),
                    std::back_inserter(fromCommand2OnlyPorts), [&](const auto &portId) {
                return std::find(data->mPorts.begin(), data->mPorts.end(), portId) ==
                        data->mPorts.end();
            });
            if (!fromCommand2OnlyPorts.empty()) {
                data2->mPorts = fromCommand2OnlyPorts;
                break;
            }
