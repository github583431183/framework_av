...
BINDER_METHOD_ENTRY(setUserIdStrategiesAffinity) \
BINDER_METHOD_ENTRY(removeUserIdStrategiesAffinity) \
                                                                           ...
                                                                           int flags)
{
    ...
    sp<AudioVolumeGroupData> data= new AudioVolumeGroupData();
    data->mGroup = group;
    data->mFlags = flags;
    command->mParam = data;
...
void AudioPolicyService::AudioCommandThread::insertCommand_l(sp<AudioCommand>& command, int delayMs)
{
    ...
    for (i = (ssize_t)mAudioCommands.size()-1; i >= 0; i--) {
        ...
        case SET_PORTS_VOLUME: {
            ...
            if (data->mIO != data2->mIO) break;
            if (data->mPorts != data2->mPorts) break;
