...
void SwAudioOutputDescriptor::setSwMute(
        bool muted, VolumeSource vs, const DeviceTypeSet& deviceTypes, uint32_t delayMs) {
                                        ...
                                        bool isVoiceVolSrc)
{
    if (!AudioOutputDescriptor::setVolume(
            volumeDb, muted, vs, deviceTypes, delayMs, force, isVoiceVolSrc)) {
        return false;
    }
    for (const auto& devicePort : devices()) {
    ...
    mClientInterface->setPortsVolume(getPortsForVolumeSource(vs), volumeAmpl, mIoHandle, delayMs);
    return true;
}
                                        ...
                                        bool isVoiceVolSrc)
{
    bool changed = AudioOutputDescriptor::setVolume(
            volumeDb, muted, volumeSource, deviceTypes, delayMs, force, isVoiceVolSrc);
