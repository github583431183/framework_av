...
bool isCriterionMatched(const AudioMixMatchCriterion& criterion,
    ...
    switch(ruleWithoutExclusion) {
        ...
        case RULE_MATCH_UID:
            {
                if (uid == 0) {
                    // Workaround until delivery of feature multi_zone_audio (b/316643994)
                    // When the AudioPolicyManager checks the applicable device for audio attributes
                    // it evaluates the rules of the dynamic policy mixes installed fisrt.
		    ...
		    //   if there is a matching / exclusion rule on UID, it reports a false negative
                    ...
                    // Fix:
		    //   if there is a matching rule on the UID and UID=0 is provided by the caller
        ...
        case RULE_MATCH_USERID:
            {
                if (uid == 0) {
                    // Workaround until delivery of feature multi_zone_audio (b/316643994)
                    // When the AudioPolicyManager checks the applicable device for audio attributes
                    // it evaluates the rules of the dynamic policy mixes installed first.
                    // If one is matching, the device returned is the registered one for this rule.
                    // For volume request, the Uid/UserId is unknown, thus the request of matching
                    // mixes is done with UID=0.
                    // Issue:
