...
static constexpr size_t kTokenId = 4294967296000;
...
std::string keyMediaValues[] = {
        "metrics.manager"
        "mediadrm" AMEDIAMETRICS_KEY_AUDIO_MIDI,
...
std::string keyMediaAction[] = {
        ...
        AMEDIAMETRICS_PROP_EVENT_VALUE_TIMEOUT AMEDIAMETRICS_PROP_EVENT_VALUE_CTOR,
        AMEDIAMETRICS_PROP_EVENT_VALUE_ENDAAUDIOSTREAM AMEDIAMETRICS_PROP_EVENT_VALUE_DEVICECLOSED,
        "createAudioPatch" AMEDIAMETRICS_PROP_EVENT_VALUE_SETVOICEVOLUME,
...
void MediaMetricsServiceFuzzer::setKeyValues(std::shared_ptr<mediametrics::Item>& item,
    ...
    auto invokeActionAPIs = mFdp.PickValueInArray<const std::function<void()>>({
            ...
            [&]() { item->setTimestamp(mFdp.ConsumeIntegral<int32_t>()); },
            [&]() {
...
void MediaMetricsServiceFuzzer::process() {
    ...
    if (mFdp.ConsumeBool()) {
        IPCThreadState::self()->restoreCallingIdentity(kTokenId);
    } else {
