...
void EffectHandle::disconnect(bool unpinIfLast)
{
    ...
    mDisconnected = true;
    {
        sp<IAfEffectBase> effect = mEffect.promote();
        if (effect != 0) {
            // Unlock e.g. for non hw accelerated device effect, need to acquire AudioFlinger lock
            _l.unlock();
