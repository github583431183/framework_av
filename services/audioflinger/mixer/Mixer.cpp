...
class DefaultMixer : public IAfMixer {
  public:
    ...
    void enable(int name) final { mAudioMixer.enable(name); }

    void disable(int name) final { mAudioMixer.disable(name); }
    ...
    void setChannelMask(int name, audio_channel_mask_t channelMask) final {
        mAudioMixer.setParameter(name, AudioMixer::TRACK, AudioMixer::CHANNEL_MASK,
                                 (void*)static_cast<uintptr_t>(channelMask));
    }

    void setFormat(int name, audio_format_t format) final {
        mAudioMixer.setParameter(name, AudioMixer::TRACK, AudioMixer::FORMAT,
                                 (void*)static_cast<uintptr_t>(format));
    }
    ...
    void setResampler(int name, uint32_t sampleRate) final {
        mAudioMixer.setParameter(name, AudioMixer::RESAMPLE, AudioMixer::SAMPLE_RATE,
                                 (void*)static_cast<uintptr_t>(sampleRate));
    }
    ...
    void setTeeBuffer(int name, void* buffer, size_t frameCount) final {
        mAudioMixer.setParameter(name, AudioMixer::TRACK, AudioMixer::TEE_BUFFER, buffer);
        if (buffer) {
            mAudioMixer.setParameter(name, AudioMixer::TRACK, AudioMixer::TEE_BUFFER_FRAME_COUNT,
                                     (void*)static_cast<uintptr_t>(frameCount));
        }
    }
